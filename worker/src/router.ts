...
function matchRoute(method: string, pathname: string): { handler: Handler; params: Record<string, string>; requireAuth: boolean } | null {
...
addRoute('PUT', '/config', async (req, env) => handleUpdateConfig(req, env), true);
...
addRoute('GET', '/admin/export', async (req, env) => handleExport(req, env), true);
addRoute('POST', '/admin/import', async (req, env) => handleImport(req, env), true);
