import{t as s,g as d}from"./i18n.Lr-IErb8.js";async function r(){const e=document.getElementById("article-grid");if(!e)return;const n=d()==="zh"?"zh-CN":"en-US";try{const t=await(await fetch("/api/v1/articles?page=1&page_size=6&status=published")).json();if(t.code===0&&t.data){const{items:c}=t.data;if(c.length===0){e.innerHTML=`<p class="empty-text" style="grid-column:1/-1;text-align:center;color:var(--color-text-tertiary);padding:3rem 0;">${s("article.no_articles")}</p>`;return}e.innerHTML=c.map(a=>`
        ...
        `).join(""),window.dispatchEvent(new CustomEvent("motion:refresh"))}}catch{e.innerHTML=`<p class="error-text" style="grid-column:1/-1;text-align:center;color:var(--color-text-tertiary);padding:3rem 0;">${s("article.load_error")}</p>`}}r();document.addEventListener("locale-changed",()=>r());
