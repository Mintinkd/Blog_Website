import{t as n,g as y}from"./i18n.Cp1GMe-U.js";import{g as h,c as f,a as b,b as w}from"./categoryColors.B-OBBTpf.js";async function i(s=1){const c=document.querySelector(".article-list-inner");if(!c)return;const p=y()==="zh"?"zh-CN":"en-US";try{const r=await(await fetch(`/api/v1/articles?page=${s}&status=published`)).json();if(r.code===0&&r.data){const{items:d,total:_,total_pages:o}=r.data;if(d.length===0){c.innerHTML=`<p style="text-align:center;color:var(--color-text-tertiary);padding:3rem 0;">${n("article.no_articles")}</p>`;return}const m=a=>String(a||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;");c.innerHTML=d.map(a=>{const t=a.category,e=h(t?.slug,t?.name),v=t?f(e):"",u=a.cover_image?`<div class="list-card-cover"><img src="${m(a.cover_image)}" alt="" loading="lazy" /></div>`:`<div class="list-card-cover list-card-cover-grad" style="${b(e)}"><span class="list-cover-cat-name" style="${w(e)}">${t?.name||"✦"}</span></div>`;return`
